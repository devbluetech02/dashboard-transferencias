...
const VENDAS_FROM = `
  ...
  WHERE n.DTSAID >= ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -7)
    AND n.DTSAID <  TRUNC(SYSDATE, 'MM')
...
FROM (
  ...
  LEFT JOIN (
    ...
    FROM (
      SELECT m.CODFILIAL, TRUNC(n.DTSAID,'MM') AS mes_venda,
             SUM(CASE WHEN n.CONDVENDA = 7 THEN m.QTCONT ELSE m.QT END) AS qtvenda
      ${VENDAS_FROM}
        AND m.CODPROD = ${cp}
      GROUP BY m.CODFILIAL, TRUNC(n.DTSAID,'MM')
    )
    GROUP BY CODFILIAL
