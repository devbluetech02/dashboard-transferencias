# Queries das tabelas negadas pelo gateway

Erro do gateway:

```
Acesso à tabela "TAB_CODBARRAS" foi negado,Acesso à tabela "PCMOV" foi negado,Acesso à tabela "PCCLIENT" foi negado,Acesso à tabela "PCNFSAID" foi negado
```

> **Causa:** 401 = tabela fora do allowlist da API key. Não é credencial. Libere `WINDOW.TAB_CODBARRAS`, `WINDOW.PCMOV`, `WINDOW.PCCLIENT`, `WINDOW.PCNFSAID` no allowlist.

Fonte: `src/lib/estoque.ts`. Abaixo, cada tabela negada e onde aparece.

| Tabela negada | Uso | Queries |
|---|---|---|
| `WINDOW.TAB_CODBARRAS` | estoque / etiquetas / lotes | 1, 2, 3, 4 |
| `WINDOW.PCMOV` | movimentação de venda | 1, 2 |
| `WINDOW.PCNFSAID` | notas de saída | 1, 2 |
| `WINDOW.PCCLIENT` | cliente (filtro `CODATV1`) | 1, 2 |

---

## 1. Consolidado nacional — `GET /api/estoque`

Usa: `TAB_CODBARRAS`, `PCMOV`, `PCNFSAID`, `PCCLIENT`.
...
Usa: `TAB_CODBARRAS`, `PCMOV`, `PCNFSAID`, `PCCLIENT` (+ `PCFILIAL`).
...
## 4. Etiquetas — `GET /api/estoque/etiquetas?codprod=<x>&lote=<l>&cidade=<c>`

Usa: `TAB_CODBARRAS` (+ `PCPRODUT`, `PCFILIAL`).
